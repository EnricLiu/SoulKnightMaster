...
class Client:
    ...
    def action(self, REQ:Request) -> str:
        try:
            ...
            headers = None
            content_length = None
            transfer_chunked = None
            
            timeout = time.time() + self.timeout
            while time.time() < timeout:
                recv = self.hosts[host].recv(4096)
                if not recv: break
                response.extend(recv)
                
                if headers is None:
                    if b'\r\n\r\n' in response:
                        headers, body = response.split(b'\r\n\r\n', 1)
                        content_length_match = search(rb'[cC]ontent-[lL]ength: (\d+)', headers)
                        transfer_chunked = b'Transfer-Encoding: chunked' in headers

                        if content_length_match:
                            content_length = int(content_length_match.group(1))
                
                if len(response) >= content_length + len(headers):
                    break
