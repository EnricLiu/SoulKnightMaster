...
def extract_features(frame):
    if not hasattr(extract_features, "model"):
        extract_features.model = FeatureExtractor().to(DEVICE)
...
class GameEnv(gym.Env):
    def __init__(self):
        ...
        self.client = Client(node_name="SKM_16448")
        ...
        self.action_space = gym.spaces.Dict({
            "discrete_0": gym.spaces.Discrete(2),  # 第0索引，离散
            "continuous_1": gym.spaces.Box(low=-1.0, high=1.0, shape=(1,), dtype=np.float32),  # 第1索引，连续
            "discrete_2": gym.spaces.Discrete(2),  # 第2索引，离散
            "discrete_3": gym.spaces.Discrete(2)  # 第3索引，离散
        })
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, Dict]:
        """ 执行动作并返回环境反馈 """
        # 1. 执行游戏动作
        self._execute_action(Action.from_raw(action))  # 需实现实际动作执行逻辑
        # 2. 获取新状态
    ...
    def _capture_game_screen(self) -> np.ndarray:#🐮🐮🐮🐮🐮
        return self.client.fetch_fb()

    def _execute_action(self, action: Action):#🐮🐮🐮🐮🐮
        return self.client.sync_action(action)
...
class SoulKnightMasterPolicy(ActorCriticPolicy):
    def __init__(self, observation_space, action_space, lr_schedule, net_arch=None, activation_fn=nn.ReLU,  *args, **kwargs):
        ...
        self.value_net = nn.ModuleDict({
                'gru': nn.GRU(input_size=1280, hidden_size=256, batch_first=True, num_layers=1),
    ...
    def forward(self, obs, deterministic=False):
        batch_size, C, H, W = obs.shape
        batch_size = batch_size//10
        features = obs.view(batch_size, 10, -1)
        action_logits = self.action_net(features)
        gru = self.value_net['gru']
        classifier = self.value_net['classifier']
        ...
        gru_out = gru_out.reshape(batch_size * 10, -1)
        value = classifier(gru_out)[-1]  # [batch_size * sequence_length, 1]
...
if __name__ == "__main__":
    ...
    start = time.time()
    print("Making Model")
    model = PPO(SoulKnightMasterPolicy, env, verbose=2, n_steps=2, batch_size=2, learning_rate=1e-3, n_epochs=10, ent_coef=0.01)
    print("Start Learning")
    model.learn(total_timesteps=1000)
    print("Model Saving")
    model.save("./ckpt/save")
