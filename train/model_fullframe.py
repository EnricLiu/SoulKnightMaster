...
class ActorModel(nn.Module):
    def __init__(self, hidden_size=256, num_layers=1, train_label=["move", "angle", "attack", "skill"]):
...
class _ValueModel(nn.Module):
    def __init__(self, hidden_size=64, num_layers=1, train_label=["move", "angle", "attack", "skill"]):
        super(_ValueModel, self).__init__()
        self.train_label = train_label
        
        
        weights = EfficientNet_B0_Weights.DEFAULT
        self.effi_net = efficientnet_b0(weights=weights)
        in_features = self.effi_net.classifier[1].in_features  # 1280 for EfficientNet V2 S
        print(f"Input size: {in_features}")
        self.effi_net.classifier = nn.Identity()  # Remove original classifier
        self.gru = nn.GRU(in_features, hidden_size, batch_first=True, num_layers=num_layers)
