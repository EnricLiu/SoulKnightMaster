...
class MainModel(nn.Module):
    def __init__(self):
        ...
        in_features = self.resnet.fc.in_features  # 获取全连接层的输入特征数
        self.resnet.fc = nn.Identity()  # 去掉原来的全连接层，提取特征

        # 二分类任务（第一、三、四、五项）
        self.fc_binary_1 = nn.Linear(in_features, 1)  # 第1项
        self.fc_binary_3 = nn.Linear(in_features, 1)  # 第3项
        self.fc_binary_4 = nn.Linear(in_features, 1)  # 第4项
        self.fc_binary_5 = nn.Linear(in_features, 1)  # 第5项
        ...
        # 回归任务（角度预测，第2项）
        self.fc_regression_2 = nn.Linear(in_features, 1)
...
# 冻结部分模型
def freeze_model_weights(model, freeze_until_layer="layer4"):
    for name, param in model.resnet.named_parameters():
        param.requires_grad = False  # 默认冻结
        if freeze_until_layer and name.startswith(freeze_until_layer):
            param.requires_grad = True  # 解冻目标层及之后的层
...
###############################################################
        ...
        torch.save(target, os.path.join(ckpt_path, f'{train_id}-loss={_loss:.3f}-e{_epoch}.pth'))
        ...
        for epoch in range(num_epochs):
            with tqdm(total=n_train, desc=f'Epoch {epoch}/{num_epochs}') as pbar:
                ...
                for i, images in enumerate(train_loader):
                    freeze_model_weights(model, freeze_until_layer=f"layer{layer_number}")
...
if __name__ == "__main__":
    ...
    train_id, train_loss, val_loss, cplt_epoches = train(
            ...
            ckpt_path=Path("../ckpt"), save_state_dict=False, save_interval=None, use_amp=True
        )
